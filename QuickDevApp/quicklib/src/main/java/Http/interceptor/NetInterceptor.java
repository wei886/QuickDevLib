...
public class NetInterceptor implements Interceptor {
    ...
    public Response intercept(Chain chain) throws IOException {
        ...
        if (!NetWorkUtils.isNetConnect(BaseApplication.mContext)) {
...
//        }
        ...
        if (NetWorkUtils.isNetConnect(BaseApplication.mContext)) {
            int maxAge = 60 * 60; // 有网络时 设置缓存超时时间�?小时
            response = response.newBuilder()
                    .removeHeader("Pragma")
                    //清除头信息，因为服务器如果不支持，会返回�?些干扰信息，不清除下面无法生�?
